...
### Create animations with your phone (Coming soon...)

[Ditto]() is an app for iOS that lets you capture motion from your camera using body tracking algorithms based on machine learning. A pose-estimation algorithm is used to record keyframes for a humanoid animation, which can be exported as a .ma (Maya), .fbx (Blender) or .anim (Unity) file.
...
A variational autoencoder is designed with [TensorFlow](https://www.tensorflow.org/tutorials/generative/cvae). Graphs for the [Encoder](TensorFlow/vae_mlp_encoder.png) and [Decoder](TensorFlow/vae_mlp_decoder.png) are available in `TensorFlow/`. After acquiring data, training can be accomplished via the command line
...
Here's a model trained on motion capture data from yoga poses. The decoder portion of the VAE can be exported to Android and iOS!

![](Figures/android_decoder.gif)

Tested on a Pixel 3a

Try it for Android: [decoder.apk](Unity/AnimationAutoencoder/decoder.apk)
