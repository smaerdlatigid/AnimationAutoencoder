...
if __name__ == '__main__':
    ...
    parser.add_argument("-b", "--batch", help=help_, default=16, type=int)
    help_ = "Dimensionality of latent space"
    parser.add_argument("-l", "--latent", help=help_, default=4, type=int)
    ...
    inp = x_test[np.random.randint(x_test.shape[0])].reshape(1,-1)
    output = vae.predict(inp)
    joints = output[0].reshape(-1,7)
    ijoints = inp[0].reshape(-1,7)
    for j in range(len(joints)):
        ax.scatter(joints[j][0], joints[j][1], joints[j][2], 'o', c='black')
        ax.scatter(ijoints[j][0], ijoints[j][1], ijoints[j][2], 'o', c='green')
    plt.show()
    # plot_results(models,
    #              data,
    #              batch_size=batch_size,
